...
export const createQuiz = async (req, res) => {
  ...
  try {
    ...
    if (!platform)
      return res
        .status(404)
        .json({ msg: `No platform with id: ${platformId}` });
    ...
    platform.quizzes.push(createdQuiz._id);
...
export const getQuiz = async (req, res) => {
  ...
  try {
    ...
    if (!quiz)
      return res
        .status(500)
        .json({ message: "Quiz not found with the provided id" });
...
export const editQuiz = async (req, res) => {
  ...
  try {
    let newQuiz = await Quiz.findByIdAndUpdate(
      quizId,
      { $set: updateFields },
      { new: true }
    );
...
export const editQuizQuestion = async (req, res) => {
  ...
  try {
    ...
    let questionIndex = quiz.questions.findIndex(
