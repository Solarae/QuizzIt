...
export const createPlatform = async (req, res) => {
  ...
  try {
    ...
    if (errors.emptyName || errors.emptyDesc)
    ...
    if (!createdPlatform)
      return res
        .status(404)
        .json({ msg: "Something went wrong with creating the platform" });
...
export const deletePlatform = async (req, res) => {
  ...
  try {
    ...
    if (!platform)
      return res.status(404).json({ msg: "Platform doesn't exist" });
...
export const getPlatform = async (req, res) => {
  try {
    const platform = await Platform.findById(req.params.id);
    if (!platform)
      return res.status(200).json({ msg: "Platform doesn't exist" });
    res.status(200).json({ platform: platform });
  } catch (error) {
    res.status(404).json({ msg: error.message });
  }
...
export const updatePlatform = async (req, res) => {
  ...
  try {
    ...
    if (!updatedPlatform)
      return res
        .status(200)
        .json({ msg: "Something went wrong with updating platform" });
